...
def val(model, val_loader, loss_func, n_labels):
    ...
    val_pixel_accuracy = metrics.pixel_acc(n_labels)
    val_precision = metrics.precision(n_labels)
    val_specificity = metrics.specificity(n_labels)
    val_recall = metrics.recall(n_labels)
    ...
    with torch.no_grad():
        for idx, (data, target) in tqdm(enumerate(val_loader), total=len(val_loader)):
            ...
            val_pixel_accuracy.update(output, target)
            val_precision.update(output, target)
            val_specificity.update(output, target)
            val_recall.update(output, target)

    val_log = OrderedDict(
        {'Val_Loss': val_loss.avg, 'Val_dice_liver': val_dice.avg[1], 'val_pixel_accuracy': val_pixel_accuracy.avg[1],
         'val_precision': val_precision.avg[1], 'val_specificity:': val_specificity.avg[1],
         'val_recall': val_recall.avg[1]})
...
def train(model, train_loader, optimizer, loss_func, n_labels, alpha):
    ...
    train_pixel_accuracy = metrics.pixel_acc(n_labels)
    train_precision = metrics.precision(n_labels)
    train_specificity = metrics.specificity(n_labels)
    train_recall = metrics.recall(n_labels)
    ...
    for idx, (data, target) in tqdm(enumerate(train_loader), total=len(train_loader)):
        ...
        train_pixel_accuracy.update(output[3], target)
        train_precision.update(output[3], target)
        train_specificity.update(output[3], target)
        train_recall.update(output[3], target)

        val_log = OrderedDict({'train_Loss': train_loss.avg, 'train_dice_liver': train_dice.avg[1],
                               'train_pixel_accuracy': train_pixel_accuracy.avg[1],
                               'train_precision': train_precision.avg[1],
                               'train_specificity:': train_specificity.avg[1], 'train_recall': train_recall.avg[1]})
