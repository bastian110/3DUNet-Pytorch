...
class LITS_preprocess:
    def __init__(self, raw_dataset_path, fixed_dataset_path, args):
        ...
        self.expand_slice = args.expand_slice  # Nombre de tranches s'étendant axialement vers l'extérieur 轴向外侧扩张的slice数量
        self.size = args.min_slices  # Nombre de tranches échantillonnées 取样的slice数量
    ...
    def fix_data(self):
        if not os.path.exists(self.fixed_path):  # Création d'un répertoire de dépôt 创建保存目录
    ...
    def process(self, ct_path, seg_path, classes=None):
        ...
        if classes == 2:
            # Fusion des étiquettes de référence pour le foie et les tumeurs hépatiques en une seule. 将金标准中肝脏和肝肿瘤的标签融合为一个
            seg_array[seg_array > 0] = 1
        # Tronquer les valeurs en niveaux de gris qui sont en dehors du seuil. 将灰度值在阈值之外的截断掉
        ct_array[ct_array > self.upper] = self.upper
        ct_array[ct_array < self.lower] = self.lower

        # Déséchantillonnage, (déséchantillonnage des axes x et y, espacement des axes de tranche normalisé à l'échelle de la tranche_down_scale) 降采样，（对x和y轴进行降采样，slice轴的spacing归一化到slice_down_scale）
        ...
        # Trouvez la tranche au début et à la fin de la région du foie et développez-la vers l'extérieur. 找到肝脏区域开始和结束的slice，并各向外扩张
        z = np.any(seg_array, axis=(1, 2))
        start_slice, end_slice = np.where(z)[0][[0, -1]]

        # Une tranche dans chaque direction 两个方向上各扩张个slice
        ...
        # Si le nombre de tranches restantes à ce stade est inférieur à la taille, abandonnez, de sorte qu'il y a très peu de données. 如果这时候剩下的slice数量不足size，直接放弃，这样的数据很少
        if end_slice - start_slice + 1 < self.size:
            print('Too little slice，give up the sample:')
            return None, None
        # Intercepter les zones réservées 截取保留区域
        ct_array = ct_array[start_slice:end_slice + 1, :, :]
        seg_array = seg_array[start_slice:end_slice + 1, :, :]
        print("Preprocessed shape:", ct_array.shape, seg_array.shape)
        # Sauvegarder dans le format correspondant 保存为对应的格式
...
if __name__ == '__main__':
    raw_dataset_path = '/content/segmentation/'
    fixed_dataset_path = '/content/fixed'

    args = config.args
    tool = LITS_preprocess(raw_dataset_path, fixed_dataset_path, args)
    tool.fix_data()  # Découpage et sauvegarde de l'image originale 对原始图像进行修剪并保存
    tool.write_train_val_name_list()  # Créer un fichier txt indexé 创建索引txt文件